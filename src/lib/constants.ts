...
export const codeProblem = {
  ...
  functionName: "twoSum",
  printing: (input: any, lang: string) => {
    switch (lang) {
      case "javascript":
        return `console.log(twoSum(${JSON.stringify(input.nums)}, ${
          input.target
        }))`;
      case "python":
        return `print(twoSum(${JSON.stringify(input.nums)}, ${input.target}))`;
      case "java":
        return `System.out.println(Arrays.toString(twoSum(${JSON.stringify(
          input.nums
        )}, ${input.target})));`;
      case "cpp":
        return `for (int i : twoSum(${JSON.stringify(input.nums)}, ${
          input.target
        })) {\n  cout << i << " ";\n}\n`;
      default:
        return "";
